...
class ActiveOrderExportCustomer implements FromCollection, ShouldAutoSize, WithEvents
{
    public function collection()
    {
        $order=Order::where('status_id', '<',7)->where('visible',1)->where('pouzivatel_id', auth()->user()->id)->orderBy('dtm_ukoncenia','desc')->get(['id','pouzivatel_id','nazov_objednavky','pocet','dtm_vytvorenia','dtm_ukoncenia','status_id','polozky','popis','poznamka','tracking_num','faktura']);
        $statuses=Status::all();
