        ...
        <ul class="app-breadcrumb breadcrumb">
            <div class="dropdown ahref">
                ...
                <div class="dropdown-menu dropdown-menu-right filter mt-5 mt-md-0">
                    <form action="{{ route('admin.closedOrders.filter', app()->getLocale()) }}" method="get" role="form" class="px-4 py-3">
                        @csrf
                        ...
                        <a id="drpClose" href="{{ route('admin.closedOrders.index' , app()->getLocale()) }}" class="btn btn-secondary">{{ __('Cancel')}}</a>

                    </form>
                </div>
            </div>
            <a href="{{ route('admin.export_excel.exportClosed', app()->getLocale()) }}" class="btn btn-primary ml-2 ml-md-2"> <i class="fa fa-fw fa-lg fa-file-excel-o"></i>{{ __('Excel') }}</a>
                            ...
                            <table data-order='[]' class="table table-hover w-100 table-bordered " id="sampleTable">
                                ...
                                @foreach($order as $row)
                                    <tr>
                                        ...
                                        <td><a href="{{route('admin.closedOrders.edit', ['id'=> $row->id,'locale'=>app()->getLocale()]) }}"><strong>{{date("dmy",strtotime($row->dtm_vytvorenia)).$row->user->user_info->krajina.sprintf("%05s", $row->id)}}</strong></a></td>
                                        <td>{{$row->user->name}}</td>
