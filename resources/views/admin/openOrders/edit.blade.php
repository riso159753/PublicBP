...
@section('content')
    ...
    <div class="app-title">
        ...
        <a href="{{ route('admin.openOrders.index', app()->getLocale()) }}" class="btn btn-primary pull-right">{{ __('Orders') }}</a>

    </div>
    <div class="col-md-12">
        <form action="{{ route('admin.openOrders.update', ['id'=> $order->id,'locale'=>app()->getLocale()]) }}" method="POST" role="form">
            @csrf
                        ...
                        <div class="tile-footer">
                            <div class="row d-print-none mt-2">
                                <div class="col-12 text-right">
                                    ...
                                    <a class="btn btn-danger" @if($order->status_id<7) href="{{ route('admin.openOrders.index', app()->getLocale()) }}" @else href="{{route('admin.closedOrders.index', app()->getLocale())}}" @endif><i class="fa fa-fw fa-lg fa-arrow-left"></i>{{ __('Go Back') }}</a>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <script>
            ...
            $('#add').click(function(){
                ...
                $('#dynamic_field').append('<tr id="row'+i+'"  style="display: table-row">' +
                    ...
                    '             <option ></option>\n'+
                    {!! json_encode(App\Http\Controllers\Admin\HomeController::generateOptionList()); !!}+
                        '             </select>\n'+
            ...
            <script>
                $('.delete-confirm').on('click', function (e) {
                    ...
                    swal({
                        title: 'Are you sure?',
                        text: 'Hovewer, record will not be permanantly deleted until it is saved!',
                        icon: 'error',
                        buttons: ["Cancel", "Yes!"],
                    }).then((Delete) => {
                        if (Delete) {
            ...
            <script>
                $('.delete-confirmSK').on('click', function (e) {
                    ...
                    swal({
                        ...
                        text: 'Ašak záznam bude vymazaný až po uložení!',
                        icon: 'error',
                        buttons: ["Zrušiť", "Áno!"],
                    }).then(function() {
