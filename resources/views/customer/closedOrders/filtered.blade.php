                        ...
                        <table data-order='[]' class="table table-hover w-100 table-bordered " id="sampleTable">
                            ...
                            @foreach($order as $row)
                                <tr>
                                    ...
                                    <td><a href="{{route('customer.closedOrders.edit', ['id'=> $row->id,'locale'=>app()->getLocale()]) }}"><strong>{{date("dmy",strtotime($row->dtm_vytvorenia)).$row->user->user_info->krajina.sprintf("%05s", $row->id)}}</strong></a></td>
                                    <td>{{$row->user->name}}</td>
